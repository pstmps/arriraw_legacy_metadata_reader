...
def load_config(file: str, keys: list = None) -> dict:
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Error loading {file}. Original error: {str(e)}"
        ) from e
...
def find_files(path: str, extensions: list, ignore_multipart: bool = True) -> list:
    ...
    if not file_paths:
        raise FileNotFoundError(
            f"No files with the extensions {' '.join(extensions)} were found."
        )
    return file_paths
...
@click.option(
    "--config", "-c", help="Path to the config file.", default="defaultconfig"
)
@click.option(
    "--outputpath", "-o", help="Path to the output file.If not specified, pwd."
)
@click.option(
    "--outputformat",
    "-ofmt",
