...
class BinaryFileDTO(ABC):
    ...
    def _read_and_unpack(
        self, input_bytes: bytes, format_string: str
        ...
        return struct.unpack(
            format_string, input_bytes.read(struct.calcsize(format_string))
        )[0]
    ...
    def _read_frameline(
        self, input_bytes: bytes, frameline_number: str, endianness: str
    ) -> dict:
        """
        ...
        """
        frameline_type = self._read_and_unpack(
            input_bytes=input_bytes, format_string="<I"
        )
        frameline = {}
    ...
    def _convert_data_to_tstop(data: int) -> str:
        ...
        """
        tstop_mapping = {-3: "NearClose", -2: "Close", -1: "Invalid"}
    ...
    def _bytes_to_time_code(time_code_bytes: bytes) -> str:
        ...
        reversed_tc = "".join(
            [hex_tc[i : i + 2] for i in range(0, len(hex_tc), 2)][::-1]
        )
        timecode = ":".join(
            [reversed_tc[i : i + 2] for i in range(0, len(reversed_tc), 2)]
        )
        return timecode
    ...
    @staticmethod
    def _bcd_to_str(
        bcd: bytes, format_string: str, spacer: str, endianness: str, prefix: str = None
    ) -> str:
        """
        ...
        if format_string == "date":
            return (
                f"{date_string[:4]}{spacer}{date_string[4:6]}{spacer}{date_string[6:8]}"
            )
    ...
    def extract_metadata(self) -> dict:
        ...
        for field in self.fields:
            if (
                self.fields_to_extract is None
                or field["name"] in self.fields_to_extract
            ):
                self.file.seek(field["offset"])
                endianness = (
                    field["endianness"]
                    if field.get("endianness", None) is not None
    ...
    def handle_field(self, field: dict, endianness: str) -> dict:
        ...
        handler = field_handlers.get(field["datatype"], self._handle_default_field)
    ...
    def _handle_string_field(self, field: dict, endianness: str) -> dict:
        ...
        """
        return {
            field["name"]: self._read_string(self.file, field["length"], endianness)
        }
    ...
    def _handle_user_string_field(self, field: dict, endianness: str) -> dict:
        ...
        Returns:
            dict: the field handled
        """
        return self._split_user_string(
            self._read_string(self.file, field["length"], endianness)
        )
    ...
    def _handle_default_field(self, field: dict, endianness: str) -> dict:
        ...
        """
        return {
            field["name"]: self._read_and_unpack(
                input_bytes=self.file, format_string=endianness + field["datatype"]
            )
        }
    ...
    def _handle_date_field(self, field: dict, endianness: str) -> dict:
        ...
        date_bcd = self.file.read(
            4
        )  # self._read_and_unpack(input_bytes=self.file, format_string= endianness + 'B')
        return {
            field["name"]: self._bcd_to_str(
                bcd=date_bcd,
